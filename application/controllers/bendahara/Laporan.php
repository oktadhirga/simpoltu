...
class Laporan extends CI_Controller {
	...
	private function _cetak_bku()
	{
		...
		$yearTo = date('Y', strtotime($this->input->get('tgB')));
		$monthTo = date('n', strtotime($this->input->get('tgB')));
		$dayTo = date('d', strtotime($this->input->get('tgB')));
		$tanggal = $dayTo.' '.$monthArray[$monthTo].' '.$yearTo;
		//cek dateFrom ? panjar
		$datePanjar = $this->laporan->search_panjar($dateFrom, $id_kegiatan);
		if ($datePanjar){
			//$list_laporan = $this->laporan->listing($datePanjar->tgl_laporan);
			$list_laporan = $this->laporan->listing($dateFrom,$dateTo);
		} else {
		...
		$saldo=$this->laporan->ceksaldosebelum($id_kegiatan,$dateFrom);

		
		$tbl .= '<tr><td style="width:4%"></td><td style="width:10%"></td><td style="width:43%">Saldo Sebelumnya</td><td style="width:10%"></td><td style="width:11%"></td><td style="width:11%"></td><td align="right" style="width:11%">'.$this->money($saldo).'</td></tr>';
		...
		foreach ($list_laporan as $laporan) {
			...
			if ($laporan->id_kegiatan == $id_kegiatan ) {
					...
					if ($dateLap >= $dateFrom && $dateLap <= $dateTo) {
						...
						//ADD TITLE FOR SPJ SAH
						if ($laporan->jenis_laporan == "panjar_sah" && $isTitle == 0){
							$panjar_sah = $this->spj_detail->get_panjar_sah($laporan->id_gabung);
							$tgl_pengesahan = date('d-m-Y', strtotime($panjar_sah->tgl_pengesahan));
							$no_pengesahan = $panjar_sah->no_pengesahan;
							$tbl .= '<tr><td align="center">'.$i.'</td><td align="center">'.$tgl_pengesahan.'</td><td>Pertanggungjawaban Uang Panjar dengan Nomor Pengesahan SPJ:'.$no_pengesahan.' dengan rincian belanja sebagai berikut:</td><td align="center"></td><td></td><td></td><td></td></tr>';
							$isTitle = 1;
							$i++;
						}

						$tbl .= '<tr><td align="center">'.$i.'</td><td align="center">'.$this->date_dmy($laporan->tgl_laporan).'</td><td>'.$laporan->ket_laporan.'</td><td align="center">'.$kode_rekening.'</td><td align="right">'.$penerimaan.'</td><td align="right">'.$pengeluaran.'</td><td align="right">'.$this->money($saldo).'</td></tr>';
						$i++;
		...
		if ($i == 1) {
			$tbl .= '<tr><td colspan="7" align="center"><br><br><br><br><br><div class="nihil">N  I  H  I  L </div><br><br><br><br><br></td></tr>';
		}
	...
	// 		TCPDF::writeHTMLCell 	($w, $h, $x, $y, $html = “, $border = 0, $ln = 0, $fill = false, $reseth = true, $align = “, $autopadding = true)
		...
		$pdf->writeHTMLCell(55,40,130,'', $foot_right, 0, 0 , 0, true, 'C', true);
	...
	private function _cetak_bkt()
	{
		...
		if ($datePanjar){
			...
			$list_laporan = $this->laporan->listing_bkt($dateFrom,$dateTo);
		} else {
		...
		foreach ($list_laporan as $lap) {
			if ($lap->id_kegiatan == $id_kegiatan ) {
				$dateLap = date('Y-m-d', strtotime($lap->tgl_a));
				if ($dateLap < $dateFrom) {
		...
		*/
		...
		$saldo_sebelum=$this->laporan->ceksaldosebelum_bkt($id_kegiatan,$dateFrom);
		$saldo_sebelumnya = $this->money($saldo_sebelum);
		$tbl .= '<tr><td style="width:4%"></td><td style="width:10%"></td><td style="width:43%">Saldo Sebelumnya</td><td style="width:10%"></td><td style="width:11%"></td><td style="width:11%"></td><td align="right" style="width:11%">'.$saldo_sebelumnya.'</td></tr>';

		//body table
		foreach ($list_laporan as $laporan) {
			if ($laporan->id_kegiatan == $id_kegiatan ) {
					$dateLap = date('Y-m-d', strtotime($laporan->tgl_a));
					if ($dateLap <= $dateTo) {
							$saldo_sebelum = $saldo_sebelum + $laporan->penerimaan - $laporan->pengeluaran;
					}
					//cek tgl_laporan
					if ($dateLap >= $dateFrom && $dateLap <= $dateTo) {
						$penerimaan = $laporan->penerimaan == 0 ? '': $this->money($laporan->penerimaan);
						...
						$tbl .= '<tr><td align="center">'.$i.'</td><td align="center">'.$this->date_dmy($laporan->tgl_a).'</td><td>'.$laporan->ket_laporan.'</td><td align="center">'.$kode_rekening.'</td><td align="right">'.$penerimaan.'</td><td align="right">'.$pengeluaran.'</td><td align="right">'.$this->money($saldo_sebelum).'</td></tr>';
						$i++;
		...
		if ($i == 1) {
			$tbl .= '<tr class="nih"><td colspan="7" align="center"><br><br><br><br><br><div class="nihil">N  I  H  I  L</div><br><br><br><br><br></td></tr>';
		}
	...
	private function _cetak_pajak()
	{
		...
		$user = $this->user->get_by_id($this->id_user);
		$i = 1;
		$saldo = $saldo_sebelum = $saldo_sebelumnya = $tPenerimaan = $tPengeluaran = 0;
		$id_kegiatan = $this->input->get('optKg');
		if ($id_kegiatan) {
			$kegiatan = $this->kegiatan->get_by_id($id_kegiatan);
		}
		...
		foreach ($list_laporan as $laporan) {
			if ($laporan->id_kegiatan == $id_kegiatan ) {
					...
					$dateLap = date('Y-m-d', strtotime($laporan->tgl_laporan));
					if ($dateLap >= $dateFrom && $dateLap <= $dateTo) {
						$tPenerimaan += $laporan->penerimaan;
						$tPengeluaran += $laporan->pengeluaran;
						$penerimaan = $laporan->penerimaan == 0 ? '': $this->money($laporan->penerimaan);
						$pengeluaran = $laporan->pengeluaran == 0 ? '': $this->money($laporan->pengeluaran);
						$tbl .= '<tr><td align="center">'.$i.'</td><td align="center">'.$this->date_dmy($laporan->tgl_laporan).'</td><td>'.$laporan->ket_laporan.'</td><td align="center">'.$laporan->kode_rekening.'</td><td align="right">'.$penerimaan.'</td><td align="right">'.$pengeluaran.'</td><td align="right">'.$this->money($saldo).'</td></tr>';
						$i++;

					}
			}
		}
		$tbl .= '<tr style="font-weight:bold"><td></td><td></td><td colspan="2">TOTAL</td><td align="right">'.$this->money($tPenerimaan).'</td><td align="right">'.$this->money($tPengeluaran).'</td><td align="right"></td></tr>';


		if ($i == 1) {
			$tbl .= '<tr><td colspan="7" align="center"> N  I  H  I  L </td></tr>';
		}
...
// 		TCPDF::writeHTMLCell 	($w, $h, $x, $y, $html = “, $border = 0, $ln = 0, $fill = false, $reseth = true, $align = “, $autopadding = true)
	...
	private function _cetak_lp()
	{
		...
		$idBelanja = $this->rekening_max->get_by_id_kegiatan($id_kegiatan);
		...
		$tbl .= '<table border="1" cellpadding="2" width="100%">
						  <thead>
							<tr align="center">
								<th style="width:6%" rowspan="2">Kode Rekening</th>
								<th style="width:21%" rowspan="2">Uraian</th>
								<th style="width:6%" rowspan="2">Jumlah Anggaran</th>
								<th style="width:18%" colspan="3">SPJ-LS Gaji</th>
								<th style="width:18%" colspan="3">SPJ-LS Barang Jasa</th>
								<th style="width:18%" colspan="3">SPJ-UP/GU/TU</th>
								<th style="width:7%" rowspan="2">Jumlah SPJ (LS+UP/GU/TU) s.d Bulan ini</th>
								<th style="width:6%" rowspan="2">Sisa Pagu Anggaran</th>
							</tr>
							<tr align="center">
								<th width="6%">s.d Bulan Lalu</th>
								<th width="6%">Bulan ini</th>
								<th width="6%">s.d Bulan ini</th>
								<th width="6%">s.d Bulan Lalu</th>
								<th width="6%">Bulan ini</th>
								<th width="6%">s.d Bulan ini</th>
								<th width="6%">s.d Bulan Lalu</th>
	  ...
	  $tbl .='<tr><td width="6%"></td><td width="21%"></td><td width="6%"></td><td width="6%"></td><td width="6%"></td><td width="6%"></td><td width="6%"></td><td width="6%"></td><td width="6%"></td><td width="6%"></td><td width="6%"></td><td width="6%"></td><td width="7%"></td><td width="6%"></td></tr>';
		...
		foreach ($idBelanja as $idBelanja) {
			...
			//hitung belanja Gaji
			$lastMonthGaji = $this->laporan->last_month_gaji($dateFrom, $idBelanja->id_rekening, $id_kegiatan);
			$thisMonthGaji = $this->laporan->this_month_gaji($dateFrom, $dateTo, $idBelanja->id_rekening, $id_kegiatan);
			$tillMonthGaji = $lastMonthGaji->pengeluaran + $thisMonthGaji->pengeluaran;
			//hitung belanja LS
			$lastMonthLs = $this->laporan->last_month_ls($dateFrom, $idBelanja->id_rekening, $id_kegiatan);
			$thisMonthLs = $this->laporan->this_month_ls($dateFrom, $dateTo, $idBelanja->id_rekening, $id_kegiatan);
			$tillMonthLs = $lastMonthLs->pengeluaran + $thisMonthLs->pengeluaran;
			//hitung belanja GU
			$lastMonthGu = $this->laporan->last_month_gu($dateFrom, $idBelanja->id_rekening, $id_kegiatan);
			$thisMonthGu = $this->laporan->this_month_gu($dateFrom, $dateTo, $idBelanja->id_rekening, $id_kegiatan);
			...
			$tbl .= '<tr align="right"><td class="uraian">'.$idBelanja->kode_rekening.'</td><td class="uraian">'.$idBelanja->uraian_rekening.'</td><td >'.$this->money($rek_max).'</td><td>'.$this->money($lastMonthGaji->pengeluaran).'</td><td>'.$this->money($thisMonthGaji->pengeluaran).'</td><td>'.$this->money($tillMonthGaji).'</td>';
		...
		$tbl .= '</tbody></table><br pagebreak = "true">';


		//INSERT TABLE HALAMAN 2
		...
		$tblpengeluaran .= '<table border="1" cellpadding="2" width="100%">
							<tr align="center">
								...
								<td style="width:21%" rowspan="2">Uraian</td>
								<td style="width:6%" rowspan="2">Jumlah Anggaran</td>
								<td style="width:18%" colspan="3">SPJ-LS Gaji</td>
								<td style="width:18%" colspan="3">SPJ-LS Barang Jasa</td>
								<td style="width:18%" colspan="3">SPJ-UP/GU/TU</td>
								<td style="width:7%" rowspan="2">Jumlah SPJ (LS+UP/GU/TU) s.d Bulan ini</td>
								<td style="width:6%" rowspan="2">Sisa Pagu Anggaran</td>
							</tr>
							<tr align="center">
		...
		//Penerimaan
		$tblpengeluaran .= '<tr><td rowspan="11"></td><td colspan="13">Penerimaan</td></tr>';
		...
		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">- Uang Panjar</td><td>'.$this->money($lastMonthSp2dGaji->penerimaan).'</td><td>'.$this->money($thisMonthSp2dGaji->penerimaan).'</td><td>'.$this->money($tillMonthSp2dGaji).'</td>';
		$tblpengeluaran .= '<td>'.$this->money($lastMonthSp2d->penerimaan).'</td><td>'.$this->money($thisMonthSp2d->penerimaan).'</td><td>'.$this->money($tillMonthSp2d).'</td>';
		$tblpengeluaran .= '<td>'.$this->money($lastMonthPanjar->penerimaan).'</td><td>'.$this->money($thisMonthPanjar->penerimaan).'</td><td>'.$this->money($tillMonthPanjar).'</td><td>'.$this->money($jumPenerimaan).'</td><td></td></tr>';
		$tblpengeluaran .= '<tr><td colspan="13">- Potongan Pajak</td></tr>';
		...
		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">  a. PPN</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>'.$this->money($lastMonthPpn->penerimaan).'</td><td>'.$this->money($thisMonthPpn->penerimaan).'</td><td>'.$this->money($tillMonthPpn).'</td><td>'.$this->money($tillMonthPpn).'</td><td></td></tr>';
		...
		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">  b. PPh 21</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>'.$this->money($lastMonthPph21->penerimaan).'</td><td>'.$this->money($thisMonthPph21->penerimaan).'</td><td>'.$this->money($tillMonthPph21).'</td><td>'.$this->money($tillMonthPph21).'</td><td></td></tr>';
		...
		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">  c. PPh 22</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>'.$this->money($lastMonthPph22->penerimaan).'</td><td>'.$this->money($thisMonthPph22->penerimaan).'</td><td>'.$this->money($tillMonthPph22).'</td><td>'.$this->money($tillMonthPph22).'</td><td></td></tr>';
		...
		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">  d. PPh 23</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>'.$this->money($lastMonthPph23->penerimaan).'</td><td>'.$this->money($thisMonthPph23->penerimaan).'</td><td>'.$this->money($tillMonthPph23).'</td><td>'.$this->money($tillMonthPph23).'</td><td></td></tr>';
		...
		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">  e. PPh Pasal 4 (2)</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>'.$this->money($lastMonthPph42->penerimaan).'</td><td>'.$this->money($thisMonthPph42->penerimaan).'</td><td>'.$this->money($tillMonthPph42).'</td><td>'.$this->money($tillMonthPph42).'</td><td></td></tr>';
		...
		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">  f. Pajak Daerah</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>'.$this->money($lastMonthPajakDaerah->penerimaan).'</td><td>'.$this->money($thisMonthPajakDaerah->penerimaan).'</td><td>'.$this->money($tillMonthPajakDaerah).'</td><td>'.$this->money($tillMonthPajakDaerah).'</td><td></td></tr>';


		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">- Lain-lain</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>';
		...
		$tblpengeluaran .= '<tr align="right"><td class="uraian" colspan="2">Jumlah Penerimaan</td><td>'.$this->money($lastMonthSp2dGaji->penerimaan).'</td><td>'.$this->money($thisMonthSp2dGaji->penerimaan).'</td><td>'.$this->money($tillMonthSp2dGaji).'</td>';
		$tblpengeluaran .= '<td>'.$this->money($lastMonthSp2d->penerimaan).'</td><td>'.$this->money($thisMonthSp2d->penerimaan).'</td><td>'.$this->money($tillMonthSp2d).'</td>';
		$tblpengeluaran .= '<td>'.$this->money($lastMonthPenerimaanPanjar).'</td><td>'.$this->money($thisMonthPenerimaanPanjar).'</td><td>'.$this->money($tillMonthPenerimaanPanjar).'</td><td>'.$this->money($jumTotalPenerimaan).'</td><td></td></tr>';
...
// 		TCPDF::writeHTMLCell 	($w, $h, $x, $y, $html = “, $border = 0, $ln = 0, $fill = false, $reseth = true, $align = “, $autopadding = true)
		...
		$pdf->Ln(5);
		$pdf->writeHTMLCell(55,30,50,'', $foot_left, 0, 0 , 0, true, 'C', true);
		$pdf->writeHTMLCell(55,30,220,'', $foot_right, 0, 0 , 0, true, 'C', true);
	...
	private function _cetak_realisasi()
	{
		...
		$tbl .= '<table border="1" cellpadding="2" width="100%">
							...
							<tr align="center">
								...
								<td style="width:10%" rowspan="2">KODE REKENING</td>
								<td style="width:22%" colspan="2">PAGU ANGGARAN KEGIATAN</td>
								<td style="width:30%" rowspan="2">URAIAN</td>
								<td style="width:22%" colspan="2">REALISASI KEGIATAN (SP2D)</td>
								<td style="width:11%" rowspan="2">SISA PAGU ANGGARAN</td>
							</tr>
							<tr align="center">
								<td>UP/GU/TU</td>
								<td>LS</td>
		...
		foreach ($idBelanja as $idBelanja) {
			...
			$tbl .= '<tr><td class="center" style="width:5%">'.$i.'</td><td style="width:10%" align="center">'.$idBelanja->kode_rekening.'</td><td class="right" style="width:11%">'.$this->money($rek_max).'</td><td class="right" style="width:11%"></td><td style="width:30%">'.$idBelanja->uraian_rekening.'</td><td class="right" style="width:11%">'.$this->money($lastMonthGu->pengeluaran).'</td><td class="right" style="width:11%">'.$this->money($lastMonthLs->pengeluaran).'</td>
							<td class="right" style="width:11%">'.$this->money($sisaPagu).'</td></tr>';
	...
	// 		TCPDF::writeHTMLCell 	($w, $h, $x, $y, $html = “, $border = 0, $ln = 0, $fill = false, $reseth = true, $align = “, $autopadding = true)
		...
		$pdf->Output('Realisasi'.$this->input->get('tgA').'.pdf', 'I');
	}
