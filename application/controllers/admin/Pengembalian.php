...
class Pengembalian extends CI_Controller {
	...
	public function ajax_list($id_panjar)
	{
		...
		foreach ($list as $pengembalian) {
			if ($pengembalian->id_panjar == $id_panjar) {

						$no++;
						$row = array();
						$row[] = $pengembalian->nama_kegiatan;
						$row[] = $pengembalian->no_bukti;
	...
	public function ajax_add()
	{
		$this->_validate();
		$data = array(
				'id_panjar' => $this->input->post('option_kegiatan'),
	...
	private function _validate()
	{
		...
		if($this->input->post('option_kegiatan') == '')
		{
			...
			$data['error_string'][] = 'Nama kegiatan/No. Panjar harus diisi';
			$data['status'] = FALSE;
		}
