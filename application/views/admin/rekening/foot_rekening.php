...
$(document).ready(function() {
      ...
      table = $('#table').DataTable({
          ...
          "ajax": {
              "url": "<?php echo base_url('admin/rekening/ajax_list')?>",
              "type": "POST"
          },
...
$('[name="option_jenis_belanja"]').change(function(){
  ...
  $.ajax({
      url : "<?php echo base_url('admin/rekening/ajax_edit') ?>/" + id_rekening,
      type: "GET",
      dataType: "JSON",
...
$('#tambah-rekening').click(function(){
      ...
      $.ajax({
          url : "<?php echo base_url('admin/rekening/option_jenis_belanja') ?>",
          type: "GET",
          dataType: "JSON",
          success: function(data)
          {
...
$('#simpan-rekening').click(function(){
  ...
  if(save_method == 'add') {
      url = "<?php echo site_url('admin/rekening/ajax_add')?>";
      notif = "<strong><h4>Info!</h4></strong>Kode rekening " + $('[name="kode_rekening"]').val()  + " berhasil ditambahkan";
  } else {
      url = "<?php echo site_url('admin/rekening/ajax_update')?>";
...
function edit_rekening(id_rekening)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $.ajax({
                url : "<?php echo base_url('admin/rekening/option_jenis_belanja_edit') ?>/" + data.id_rekening,
                type: "GET",
                dataType: "JSON",
                success: function(data)
                {
    ...
    $.ajax({
        ...
        success: function(data)
        {
          ...
          if(confirm('Anda yakin akan menghapus kode rekening' + data.kode_rekening + ' ?'))
          {
              ...
              $.ajax({
                  url : "<?php echo base_url('admin/rekening/ajax_delete/')?>/"+id_rekening,
                  type: "POST",
                  dataType: "JSON",
                  success: function(data)
                  {
